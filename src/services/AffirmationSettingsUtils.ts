...
export const handleStreakUpdate = async (settings: Partial<AffirmationSettingsType>, settingsService: AffirmationSettingsService, callback: React.Dispatch<React.SetStateAction<AffirmationSettingsType | null>>) => {
    if (settings.id && settings.current_streak && settings.last_practice_date) {
